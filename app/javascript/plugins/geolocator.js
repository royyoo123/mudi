const locateCoords = () => {
  ...
  maximumAge: 0
	...
	const handleButtonClick = (event) => {
		window.location.href=`https://localhost:3000/events`;
	}
	...
	function success(pos) {
	  ...
	  if(a){
	  	...
	  	a.addEventListener('click',(event) => {
			window.location.href=`https://localhost:3000/events?latitude=${crd.latitude}&longitude=${crd.longitude}`;
			})
		} else if (b) {
			b.addEventListener('click',(event) => {
			window.location.href=`https://localhost:3000/events/map?latitude=${crd.latitude}&longitude=${crd.longitude}`;
			})
		} else if (c) {
